...
public class FcmPushService : IFcmPushService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            _logger.LogDebug("FCM: Firebase не инициализирован, push-уведомление пропущено");
            return;
        }
        ...
        try
        {
            ...
            if (response.FailureCount > 0)
            {
                ...
                {
                    _logger.LogWarning("Ошибка отправки FCM: {Error}", sendResp.Exception?.Message);
                }
            }

            _logger.LogInformation(
                "FCM: отправлено {SuccessCount} из {Total} пользователю {UserId}, messageId={MessageId}",
        ...
        {
            _logger.LogError(ex, "FCM: ошибка при отправке push пользователю {UserId}", message.UserId);
        }
    }
}
