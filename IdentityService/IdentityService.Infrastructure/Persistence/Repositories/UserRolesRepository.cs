...
public class UserRolesRepository : IUserRolesRepository
{
    ...
    public async Task<List<UserRoles>> GetRolesIdByUserId(Guid userId) =>
        await _context.UserRoles.Where(x => x.UserId == userId).ToListAsync();

    public async Task<List<Roles>> GetUserRolesAsync(int roleId) =>
        await _context.Roles.Where(x => x.Id == roleId).ToListAsync();
    ...
    public async Task<int> GetMaxRole(Guid userId)
    {
        ...
        if (roleIds.Count == 0) return (int)RolesEnum.User;
        if (roleIds.Contains((int)RolesEnum.Admin)) return (int)RolesEnum.Admin;
        if (roleIds.Contains((int)RolesEnum.Master)) return (int)RolesEnum.Master;
