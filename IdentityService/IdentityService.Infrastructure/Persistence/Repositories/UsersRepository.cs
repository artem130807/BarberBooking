...
public class UsersRepository : IUserRepository
{
    ...
    public async Task<Users?> GetUserByPhone(string phone) =>
        await _context.Users.FirstOrDefaultAsync(x => x.Phone.Number == phone);
    ...
    public async Task<HashSet<PermissionsEnum>> GetUserPermissions(Guid userId)
    {
        var roles = await _context.Users.AsNoTracking()
            ...
            .Select(x => x.Roles)
            .ToListAsync();
        return roles.SelectMany(x => x).SelectMany(x => x.Permissions)
            .Select(x => (PermissionsEnum)x.Id).ToHashSet();
    }
    ...
    public async Task<Users?> GetUserById(Guid id) => await _context.Users.FindAsync(id);

    public async Task<Users?> GetUserByEmail(string email) =>
        await _context.Users.FirstOrDefaultAsync(x => x.Email == email);

    public async Task<Dictionary<Guid, string>> GetUsersByIds(IEnumerable<Guid> userIds) =>
        await _context.Users.Where(u => userIds.Contains(u.Id)).ToDictionaryAsync(u => u.Id, u => u.Name);
}
